...
class RaylibJs {
    ...
    Vector2Subtract(result_ptr, v1_ptr, v2_ptr) {
        const buffer = this.wasm.instance.exports.memory.buffer;
    ...
    Vector2Normalize(result_ptr, v_ptr) {
        const buffer = this.wasm.instance.exports.memory.buffer;
        const [x, y] = new Float32Array(buffer, v_ptr, 2);
        const len = Math.sqrt(x*x + y*y);
        new Float32Array(buffer, result_ptr, 2).set([x/len, y/len]);
    }
    ...
    Vector2Add(result_ptr, v1_ptr, v2_ptr) {
        const buffer = this.wasm.instance.exports.memory.buffer;
        const [x1, y1] = new Float32Array(buffer, v1_ptr, 2);
        const [x2, y2] = new Float32Array(buffer, v2_ptr, 2);
        new Float32Array(buffer, result_ptr, 2).set([x1 + x2, y1 + y2]);
