...
#define FROM_SCREEN_TO_WORLD(x) ((x) / 500.0f)
#define FROM_WORLD_TO_SCREEN(x) ((x) * 500.0f)
...
#define MIN_H 0.1f
#define MAX_H 0.5f
#define KERNEL_TYPE GAUSSIAN_KERNEL
...
void DrawPressureGradient(struct particle_array *particles, float h) {
    ...
    for (int i = 0; i < particles->count; i++) {
        ...
        Vector2 screen_pressure_gradient = (Vector2){
            pressure_gradient.x / 100.0,
            pressure_gradient.y / 100.0
        };
